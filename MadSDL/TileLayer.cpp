	...
	: m_TileSize(tilesize), m_RowCount(rowcount), m_ColCount(colcount), m_Tilemap(tilemap), m_Tilesets (tilesets)
{
	...
	{
		TextureManager::get().Load(m_Tilesets[i].Name, "assets/Maps/" + m_Tilesets[i].Source);
	}
}
	...
	for (unsigned int i = 0; i < m_RowCount; i++)
	{
		for (unsigned int j = 0; j < m_ColCount; j++)
		{
			...
			else
			{
				...
				TextureManager::get().DrawTile(ts.Name, ts.TileSize, j * ts.TileSize, i * ts.TileSize, tileRow, tileCol);
			}
		}
	}
}
